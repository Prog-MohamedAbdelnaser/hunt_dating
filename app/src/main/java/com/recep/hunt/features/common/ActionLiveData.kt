...
class ActionLiveData<T> : MutableLiveData<T>() {
    ...
    override fun observe(owner: LifecycleOwner, observer: Observer<in T>) {
        ...
        super.observe(owner, Observer { data ->
            ...
            // Therefore it only emits to the observer the single action so you are free to showSnackbarWithDismissAction messages over and over again
            // Or launch an activity/dialog or anything that should only happen once per action / clickPickedPlace :).
        })
    }
