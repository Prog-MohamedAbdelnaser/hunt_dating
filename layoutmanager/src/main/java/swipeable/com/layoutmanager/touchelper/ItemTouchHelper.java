...
public class ItemTouchHelper extends RecyclerView.ItemDecoration
  ...
  private final OnItemTouchListener mOnItemTouchListener = new OnItemTouchListener() {
    ...
    @Override public boolean onInterceptTouchEvent(RecyclerView recyclerView, MotionEvent event) {
      ...
      if (action == MotionEvent.ACTION_DOWN) {
        ...
        mCallback.onMoveMotionEventCanceld();
      ...
      mCallback.onMoveMotionEventvVerticallyChange(yEvent,event.getRawY());
  ...
  public void swipe(final ViewHolder viewHolder, final int direction) {
    ...
    int targetX = 0;
    int targetY = 0;
    Log.i("swipe","swipe direction "+direction);
    if (direction == UP) {
  ...
  void updateDxDy(MotionEvent ev, int directionFlags, int pointerIndex) {
   ...
   int x_cord = (int) ev.getRawX();
    // Calculate the distance moved
    mDx = x - mInitialTouchX;


    if ((x_cord-xEvent) >= 30 || (x_cord-xEvent) <= -30){

      if (x_cord>xEvent){
        mCallback.onMoveMotionEventRight(xEvent,x_cord);
      }else  if (x_cord<xEvent){
        mCallback.onMoveMotionEventLeft(xEvent,x_cord);
      }else {
        mCallback.onMoveMotionEventCanceld();
      }
...
/*    if ((directionFlags & UP) ==0) {
      Log.i("updateDxDyUP","event"+ (ev.getRawY()));

      if (ev.getRawY()<600)
      mDy = Math.max(0, mDy);
    }
    if ((directionFlags & DOWN) ==0) {
      Log.i("updateDxDyDOWN","event"+ (ev.getRawY()));
  ...
  @SuppressWarnings("UnusedParameters") public abstract static class Callback {
    ...
    public abstract void onMoveMotionEventRight(float x,float xCord);

    public abstract void onMoveMotionEventLeft(float x,float xCord);

    public abstract void onMoveMotionEventCanceld();

    public abstract void onMoveMotionEventvVerticallyChange(float y ,float yCord);
